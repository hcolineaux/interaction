...
\date{Last compiled on 20 septembre, 2023}
...
\hypertarget{regressionlogistique}{%
\section{Régression logistique}\label{regressionlogistique}}
...
\NormalTok{    out.table[}\StringTok{"A1=0"}\NormalTok{,}\StringTok{"A2=0"}\NormalTok{] }\OtherTok{\textless{}{-}} \FunctionTok{paste0}\NormalTok{(}\StringTok{"$p\_\{00\}$="}\NormalTok{,}
                                       \FunctionTok{round}\NormalTok{(int.r}\SpecialCharTok{$}\NormalTok{p[}\FunctionTok{which}\NormalTok{(int.r}\SpecialCharTok{$}\NormalTok{A1}\SpecialCharTok{==}\DecValTok{0} \SpecialCharTok{\&}\NormalTok{ int.r}\SpecialCharTok{$}\NormalTok{A2}\SpecialCharTok{==}\DecValTok{0}\NormalTok{)], }
                                             \AttributeTok{digits =} \DecValTok{3}\NormalTok{), }\StringTok{" ["}\NormalTok{,}
...
\NormalTok{                                                                     int.r}\SpecialCharTok{$}\NormalTok{A2}\SpecialCharTok{==}\DecValTok{1}\NormalTok{)], }
                                                 \AttributeTok{digits =} \DecValTok{2}\NormalTok{), }\StringTok{" ["}\NormalTok{,}
...
Les différentes quantités causales d'intérêt dans une analyse d'interaction peuvent être définies à partir des paramètres d'un modèle structurel marginal (\emph{marginal structural model}, noté MSM).
...
\begin{equation}
  \mathbb{E}(Y_{A_1,A_2}) = \beta_0 + \beta_1 A_1 + \beta_2 A_2 + \beta_3 (A_1 \times A_2) \label{eq:MSMinteraction}
\end{equation}
...
\tightlist
\item
  Pour estimer les paramètres d'un MSM par g-computation, il est d'abord nécessaire de constituer une base de données constituée :
\end{enumerate}
...
Initialement, la base de données contient 10 000 individus. Les \emph{potential outcomes} \(\overline{Q}_i(a_1,a_2,L)\) ont été estimés à partir de 4 scénarios contrefactuels. On va donc constituer une base de données de 40 000 lignes.
...
\tightlist
\item
  On peut ensuite estimer les paramètres \(\beta\) du MSM en appliquant directement l'équation \eqref{eq:MSMinteraction} sur la nouvelle base de données de 40000 lignes.
\end{enumerate}

\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\# 1) Constituer la base de données pour estimer les paramètres du MSM}
\CommentTok{\# On récupère les Qbar\_i prédits précédents, que l\textquotesingle{}on fusionne dans un}
\CommentTok{\# même vecteur }
\NormalTok{Y }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(Qbar\_00, Qbar\_10, Qbar\_01, Qbar\_11)}
\FunctionTok{length}\NormalTok{(Y)}
\CommentTok{\# on a un vecteur de 40000 lignes}

\CommentTok{\# On récupère les valeurs d\textquotesingle{}exposition qui ont servi dans les scénarios contrefactuels}
\CommentTok{\# (garder le même ordre que pour les Qbar)}
\NormalTok{X }\OtherTok{\textless{}{-}} \FunctionTok{rbind}\NormalTok{(}\FunctionTok{subset}\NormalTok{(df.A1\_0.A2\_0, }\AttributeTok{select =} \FunctionTok{c}\NormalTok{(}\StringTok{"A1"}\NormalTok{, }\StringTok{"A2"}\NormalTok{)), }\CommentTok{\# A1 = 0, A2 = 0}
           \FunctionTok{subset}\NormalTok{(df.A1\_1.A2\_0, }\AttributeTok{select =} \FunctionTok{c}\NormalTok{(}\StringTok{"A1"}\NormalTok{, }\StringTok{"A2"}\NormalTok{)), }\CommentTok{\# A1 = 1, A2 = 0}
           \FunctionTok{subset}\NormalTok{(df.A1\_0.A2\_1, }\AttributeTok{select =} \FunctionTok{c}\NormalTok{(}\StringTok{"A1"}\NormalTok{, }\StringTok{"A2"}\NormalTok{)), }\CommentTok{\# A1 = 0, A2 = 1}
           \FunctionTok{subset}\NormalTok{(df.A1\_1.A2\_1, }\AttributeTok{select =} \FunctionTok{c}\NormalTok{(}\StringTok{"A1"}\NormalTok{, }\StringTok{"A2"}\NormalTok{))) }\CommentTok{\# A1 = 1, A2 = 1}
\CommentTok{\#  dim(X) \# il s\textquotesingle{}agit d\textquotesingle{}une matrice de 40000 lignes et 2 colonnes}

\DocumentationTok{\#\# 2) Estimer les paramètres du MSM en appliquant l\textquotesingle{}équation (1.10) sur cette }
\DocumentationTok{\#\#    nouvelle base de donnée \textquotesingle{}data.frame(Y,X)\textquotesingle{}}
\CommentTok{\# Modèle structurel marginal}
\NormalTok{msm }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(Y }\SpecialCharTok{\textasciitilde{}}\NormalTok{ A1 }\SpecialCharTok{+}\NormalTok{ A2 }\SpecialCharTok{+}\NormalTok{ A1}\SpecialCharTok{:}\NormalTok{A2,}
           \AttributeTok{data =} \FunctionTok{data.frame}\NormalTok{(Y,X),}
           \AttributeTok{family =} \StringTok{"gaussian"}\NormalTok{) }
\CommentTok{\# note : il n\textquotesingle{}est pas utile d\textquotesingle{}ajuster sur les facteurs de confusion L}
\CommentTok{\# car les Y ont été prédits à partir de modèles qui les déjà ont pris en compte}
\NormalTok{msm}

\DocumentationTok{\#\# 3) Utilisation des paramètres du MSM afin d\textquotesingle{}estimer les quantités d\textquotesingle{}intérêt}
\DocumentationTok{\#\#    pour l\textquotesingle{}analyse d\textquotesingle{}interaction}
\CommentTok{\# présentation des résultats dans un tableau des effets marginaux}
...
\CommentTok{\# Estimation de p\_00, p\_10, p\_01 et p\_11 à partir des coefficients du MSM}
\NormalTok{results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ msm}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"(Intercept)"}\NormalTok{]}
\NormalTok{results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 1"}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ (msm}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"(Intercept)"}\NormalTok{] }\SpecialCharTok{+}
\NormalTok{                                     msm}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"A2"}\NormalTok{])}
\NormalTok{results.MSM[}\StringTok{"A1 = 1"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ (msm}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"(Intercept)"}\NormalTok{] }\SpecialCharTok{+}
\NormalTok{                                     msm}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"A1"}\NormalTok{])}
\NormalTok{results.MSM[}\StringTok{"A1 = 1"}\NormalTok{,}\StringTok{"A2 = 1"}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ (msm}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"(Intercept)"}\NormalTok{] }\SpecialCharTok{+}
\NormalTok{                                     msm}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"A2"}\NormalTok{] }\SpecialCharTok{+}\NormalTok{ msm}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"A1"}\NormalTok{] }\SpecialCharTok{+} 
\NormalTok{                                     msm}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"A1:A2"}\NormalTok{])}

\CommentTok{\# RD et RR dans les strates de A2}
...
\CommentTok{\# RD et RR dans les strates de A1}
...
\NormalTok{RD.interaction }\OtherTok{\textless{}{-}}\NormalTok{ msm}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"A1:A2"}\NormalTok{]}
\NormalTok{RR.interaction }\OtherTok{\textless{}{-}}\NormalTok{ ((results.MSM[}\StringTok{"A1 = 1"}\NormalTok{,}\StringTok{"A2 = 1"}\NormalTok{] }\SpecialCharTok{*}\NormalTok{ results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{]) }\SpecialCharTok{/} 
\NormalTok{                     (results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 1"}\NormalTok{] }\SpecialCharTok{*}\NormalTok{ results.MSM[}\StringTok{"A1 = 1"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{]))}
\NormalTok{RERI }\OtherTok{\textless{}{-}}\NormalTok{ ((results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{] }\SpecialCharTok{{-}}\NormalTok{ results.MSM[}\StringTok{"A1 = 1"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{] }\SpecialCharTok{{-}} 
\NormalTok{           results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 1"}\NormalTok{] }\SpecialCharTok{+}\NormalTok{ results.MSM[}\StringTok{"A1 = 1"}\NormalTok{,}\StringTok{"A2 = 1"}\NormalTok{]) }\SpecialCharTok{/} 
\NormalTok{           results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
...
\hline
A1 = 0 & 0.106 & 0.198 & 0.092 & 1.872\\
\hline
A1 = 1 & 0.404 & 0.892 & 0.488 & 2.206\\
\hline
RD within strata of A2 & 0.298 & 0.694 & NA & NA\\
\hline
RR within strata of A2 & 3.816 & 4.498 & NA & NA\\
\hline
\multicolumn{5}{l}{\rule{0pt}{1em}\textit{Note: }}\\
\multicolumn{5}{l}{\rule{0pt}{1em}additive Interaction = 0.395}\\
\multicolumn{5}{l}{\rule{0pt}{1em}RERI = 3.736}\\
\multicolumn{5}{l}{\rule{0pt}{1em}multiplicative Interaction = 1.18}\\
\end{tabular}
\end{table}

\hypertarget{estimation-du-msm-par-iptw}{%
\subsection{Estimation du MSM par IPTW}\label{estimation-du-msm-par-iptw}}

Les paramètres du MSM de l'équation \eqref{eq:MSMinteraction} peuvent également être estimée par une méthode de pondération inverse (IPTW).

Pour cela, il suffit d'appliquer une régression pondérée correspondant à l'équation \eqref{eq:MSMinteraction}, où chaque individu \(i\) est pondéré par \(w_i\) le produit l'inverse des scores de propension de l'exposition \(A_1\) et de l'exposition \(A_2\) (pour rappel, d'après le DAG simulant les données, les parents de \(A_1\) sont \(L_1\) et \(L_2\), et les parents de \(A_2\) sont \(L_2\) et \(L_3\)) :
\begin{equation*}
...
\tightlist
\item
  \(g^*(A_i)\) sont des fonctions choisies par l'utilisateur, par exemple \(g^*(A_i) = 1\) correspondant à une pondération IPTW simple, ou encore \(g^*(A_i) = g(A_i)\) qui permet de stabiliser les estimations (\emph{stabilized IPTW})
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\#\# Par exemple pour calculer des poids stabilisés : }
\DocumentationTok{\#\# 1) estimer les fonctions g(A\_1)*, g(A\_1 | L1, L2), g(A\_2*), g(A\_2 | L2, L3)}
\NormalTok{model\_gA1\_star }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(A1 }\SpecialCharTok{\textasciitilde{}} \DecValTok{1}\NormalTok{, }\AttributeTok{data =}\NormalTok{ df, }\AttributeTok{family =} \StringTok{"binomial"}\NormalTok{)}
\NormalTok{model\_gA1 }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(A1 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ L1 }\SpecialCharTok{+}\NormalTok{ L2, }\AttributeTok{data =}\NormalTok{ df, }\AttributeTok{family =} \StringTok{"binomial"}\NormalTok{)}
\NormalTok{p\_A1is1\_star }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(model\_gA1\_star, }\AttributeTok{type =} \StringTok{"response"}\NormalTok{)}
\NormalTok{p\_A1is1 }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(model\_gA1, }\AttributeTok{type =} \StringTok{"response"}\NormalTok{)}

\NormalTok{model\_gA2\_star }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(A2 }\SpecialCharTok{\textasciitilde{}} \DecValTok{1}\NormalTok{, }\AttributeTok{data =}\NormalTok{ df, }\AttributeTok{family =} \StringTok{"binomial"}\NormalTok{)}
\NormalTok{model\_gA2 }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(A2 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ L2 }\SpecialCharTok{+}\NormalTok{ L3, }\AttributeTok{data =}\NormalTok{ df, }\AttributeTok{family =} \StringTok{"binomial"}\NormalTok{)}
\NormalTok{p\_A2is1\_star }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(model\_gA2\_star, }\AttributeTok{type =} \StringTok{"response"}\NormalTok{)}
\NormalTok{p\_A2is1 }\OtherTok{\textless{}{-}} \FunctionTok{predict}\NormalTok{(model\_gA2, }\AttributeTok{type =} \StringTok{"response"}\NormalTok{)}

\NormalTok{gA1\_star }\OtherTok{\textless{}{-}}\NormalTok{ gA1 }\OtherTok{\textless{}{-}} \FunctionTok{rep}\NormalTok{(}\ConstantTok{NA}\NormalTok{, }\FunctionTok{nrow}\NormalTok{(df))}
\NormalTok{gA1\_star[df}\SpecialCharTok{$}\NormalTok{A1 }\SpecialCharTok{==} \DecValTok{1}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ p\_A1is1\_star[df}\SpecialCharTok{$}\NormalTok{A1 }\SpecialCharTok{==} \DecValTok{1}\NormalTok{]}
\NormalTok{gA1\_star[df}\SpecialCharTok{$}\NormalTok{A1 }\SpecialCharTok{==} \DecValTok{0}\NormalTok{] }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ p\_A1is1\_star[df}\SpecialCharTok{$}\NormalTok{A1 }\SpecialCharTok{==} \DecValTok{0}\NormalTok{]}
\NormalTok{gA1[df}\SpecialCharTok{$}\NormalTok{A1 }\SpecialCharTok{==} \DecValTok{1}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ p\_A1is1[df}\SpecialCharTok{$}\NormalTok{A1 }\SpecialCharTok{==} \DecValTok{1}\NormalTok{]}
\NormalTok{gA1[df}\SpecialCharTok{$}\NormalTok{A1 }\SpecialCharTok{==} \DecValTok{0}\NormalTok{] }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ p\_A1is1[df}\SpecialCharTok{$}\NormalTok{A1 }\SpecialCharTok{==} \DecValTok{0}\NormalTok{]}

\NormalTok{gA2\_star }\OtherTok{\textless{}{-}}\NormalTok{ gA2 }\OtherTok{\textless{}{-}} \FunctionTok{rep}\NormalTok{(}\ConstantTok{NA}\NormalTok{, }\FunctionTok{nrow}\NormalTok{(df))}
\NormalTok{gA2\_star[df}\SpecialCharTok{$}\NormalTok{A2 }\SpecialCharTok{==} \DecValTok{1}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ p\_A2is1\_star[df}\SpecialCharTok{$}\NormalTok{A2 }\SpecialCharTok{==} \DecValTok{1}\NormalTok{]}
\NormalTok{gA2\_star[df}\SpecialCharTok{$}\NormalTok{A2 }\SpecialCharTok{==} \DecValTok{0}\NormalTok{] }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ p\_A2is1\_star[df}\SpecialCharTok{$}\NormalTok{A2 }\SpecialCharTok{==} \DecValTok{0}\NormalTok{]}
\NormalTok{gA2[df}\SpecialCharTok{$}\NormalTok{A2 }\SpecialCharTok{==} \DecValTok{1}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ p\_A2is1[df}\SpecialCharTok{$}\NormalTok{A2 }\SpecialCharTok{==} \DecValTok{1}\NormalTok{]}
\NormalTok{gA2[df}\SpecialCharTok{$}\NormalTok{A2 }\SpecialCharTok{==} \DecValTok{0}\NormalTok{] }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ p\_A2is1[df}\SpecialCharTok{$}\NormalTok{A2 }\SpecialCharTok{==} \DecValTok{0}\NormalTok{]}

\CommentTok{\# On peut alors calculer les poids individuels stabilisés}
\NormalTok{sw }\OtherTok{\textless{}{-}}\NormalTok{ (gA1\_star }\SpecialCharTok{/}\NormalTok{ gA1) }\SpecialCharTok{*}\NormalTok{ (gA2\_star }\SpecialCharTok{/}\NormalTok{ gA2)}

\DocumentationTok{\#\# 2) Estimer les coefficients du MSM à l\textquotesingle{}aide d\textquotesingle{}une régression pondérée}
\CommentTok{\# L\textquotesingle{}équation (1.10) correspond à un modèle additif, }
\CommentTok{\# nous allons utiliser une régression linéaire (family = gaussian)}
\NormalTok{msm\_iptw }\OtherTok{\textless{}{-}} \FunctionTok{glm}\NormalTok{(Y }\SpecialCharTok{\textasciitilde{}}\NormalTok{ A1 }\SpecialCharTok{+}\NormalTok{ A2 }\SpecialCharTok{+}\NormalTok{ A1}\SpecialCharTok{:}\NormalTok{A2, }\AttributeTok{data =}\NormalTok{ df, }\AttributeTok{family =} \StringTok{"gaussian"}\NormalTok{,}
...
\CommentTok{\# Estimation de p\_00, p\_10, p\_01 et p\_11 à partir des coefficients du MSM}
\NormalTok{results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ msm\_iptw}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"(Intercept)"}\NormalTok{]}
\NormalTok{results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 1"}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ (msm\_iptw}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"(Intercept)"}\NormalTok{] }\SpecialCharTok{+}
\NormalTok{                                     msm\_iptw}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"A2"}\NormalTok{])}
\NormalTok{results.MSM[}\StringTok{"A1 = 1"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ (msm\_iptw}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"(Intercept)"}\NormalTok{] }\SpecialCharTok{+}
\NormalTok{                                     msm\_iptw}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"A1"}\NormalTok{])}
\NormalTok{results.MSM[}\StringTok{"A1 = 1"}\NormalTok{,}\StringTok{"A2 = 1"}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ (msm\_iptw}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"(Intercept)"}\NormalTok{] }\SpecialCharTok{+}
\NormalTok{                                     msm\_iptw}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"A2"}\NormalTok{] }\SpecialCharTok{+} 
\NormalTok{                                     msm\_iptw}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"A1"}\NormalTok{] }\SpecialCharTok{+} 
\NormalTok{                                     msm\_iptw}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"A1:A2"}\NormalTok{])}
...
\NormalTok{results.MSM }\OtherTok{\textless{}{-}} \FunctionTok{round}\NormalTok{(results.MSM,}\DecValTok{3}\NormalTok{)}
\NormalTok{RD.interaction }\OtherTok{\textless{}{-}}\NormalTok{ msm\_iptw}\SpecialCharTok{$}\NormalTok{coefficients[}\StringTok{"A1:A2"}\NormalTok{]}
\NormalTok{RR.interaction }\OtherTok{\textless{}{-}}\NormalTok{ ((results.MSM[}\StringTok{"A1 = 1"}\NormalTok{,}\StringTok{"A2 = 1"}\NormalTok{] }\SpecialCharTok{*}\NormalTok{ results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{]) }\SpecialCharTok{/} 
\NormalTok{                     (results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 1"}\NormalTok{] }\SpecialCharTok{*}\NormalTok{ results.MSM[}\StringTok{"A1 = 1"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{]))}
\NormalTok{RERI }\OtherTok{\textless{}{-}}\NormalTok{ ((results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{] }\SpecialCharTok{{-}}\NormalTok{ results.MSM[}\StringTok{"A1 = 1"}\NormalTok{,}\StringTok{"A2 = 0"}\NormalTok{] }\SpecialCharTok{{-}} 
\NormalTok{           results.MSM[}\StringTok{"A1 = 0"}\NormalTok{,}\StringTok{"A2 = 1"}\NormalTok{] }\SpecialCharTok{+}\NormalTok{ results.MSM[}\StringTok{"A1 = 1"}\NormalTok{,}\StringTok{"A2 = 1"}\NormalTok{]) }\SpecialCharTok{/} 
...
\hline
  & A2 = 0 & A2 = 1 & RD within strata of A1 & RR within strata of A1\\
\hline
A1 = 0 & 0.102 & 0.199 & 0.097 & 1.952\\
\hline
A1 = 1 & 0.409 & 0.900 & 0.491 & 2.202\\
\hline
RD within strata of A2 & 0.307 & 0.701 & NA & NA\\
\hline
RR within strata of A2 & 4.000 & 4.514 & NA & NA\\
\hline
\multicolumn{5}{l}{\rule{0pt}{1em}\textit{Note: }}\\
\multicolumn{5}{l}{\rule{0pt}{1em}additive Interaction = 0.394}\\
\multicolumn{5}{l}{\rule{0pt}{1em}RERI = 3.863}\\
\multicolumn{5}{l}{\rule{0pt}{1em}multiplicative Interaction = 1.13}\\
\end{tabular}
\end{table}
...
Nous avons développé un package \texttt{MargIntTmle} disponible sur cette \href{https://github.com/benoitlepage/MargIntTmle}{page Github}, qui permet de faire ces calculs automatiquement. Vous trouverez la documentation complète du package sur la page.

Ci-dessous nous illustrons l'utilisation du package \texttt{MargIntTmle} pour obtenir des estimations par g-computation, IPTW, ou TMLE.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# L\textquotesingle{}installation du package peut se faire depuis la commande }
\CommentTok{\# devtools::install\_github("benoitlepage/MargIntTmle")}
\FunctionTok{library}\NormalTok{(MargIntTmle)}

\CommentTok{\# le package \textquotesingle{}MargIntTmle\textquotesingle{} repose sur les package \textquotesingle{}ltmle\textquotesingle{} et \textquotesingle{}SuperLearner\textquotesingle{}}
\FunctionTok{require}\NormalTok{(ltmle)}
\FunctionTok{require}\NormalTok{(SuperLearner)}

\DocumentationTok{\#\# Parmi les arguments nécessaires, il faut indiquer les fonctions Q et g}
\CommentTok{\# Fonction Q}
\NormalTok{Q\_formulas }\OtherTok{=} \FunctionTok{c}\NormalTok{(}\AttributeTok{Y=}\StringTok{"Q.kplus1 \textasciitilde{} A1 + A2 + A1 * A2 + L1 + L2 + L3"}\NormalTok{)}
\CommentTok{\# Les deux fonctions g }
\NormalTok{g\_formulas }\OtherTok{=} \FunctionTok{c}\NormalTok{(}\StringTok{"A1 \textasciitilde{} L1 + L2"}\NormalTok{,}
               \StringTok{"A2 \textasciitilde{} L2 + L3"}\NormalTok{)}

\DocumentationTok{\#\# Choisir des librairies de machine learning disponibles dans le package \textquotesingle{}SuperLearner\textquotesingle{}:}
\NormalTok{SL.library }\OtherTok{=} \FunctionTok{list}\NormalTok{(}\AttributeTok{Q=}\FunctionTok{list}\NormalTok{(}\StringTok{"SL.glm"}\NormalTok{),}\AttributeTok{g=}\FunctionTok{list}\NormalTok{(}\StringTok{"SL.glm"}\NormalTok{))}

\DocumentationTok{\#\# Utilisation de la fonction \textquotesingle{}int.ltmleMSM()\textquotesingle{}}
\CommentTok{\# Cette fonction va estimer les paramètres du MSM soit par g{-}computation, }
\CommentTok{\# soit par TMLE et IPTW}

\CommentTok{\# Pour obtenir une estimation des paramètres du MSM par TMLE et par IPTW, l\textquotesingle{}argument }
\CommentTok{\# \textquotesingle{}gcomp\textquotesingle{} doit égal à FALSE}
\NormalTok{interaction.ltmle }\OtherTok{\textless{}{-}} \FunctionTok{int.ltmleMSM}\NormalTok{(}\AttributeTok{data =}\NormalTok{ df,}
                                  ...
                                  \AttributeTok{gcomp =} \ConstantTok{FALSE}\NormalTok{, }\CommentTok{\# pour l\textquotesingle{}estimation par TMLE et IPTW}
...
\CommentTok{\# Pour obtenir une estimation des paramètres du MSM par g{-}computation, l\textquotesingle{}argument}
\CommentTok{\# \textquotesingle{}gcomp\textquotesingle{} doit être égal à TRUE. }
\CommentTok{\# Les IC95\% seront alors estimés par bootstrap, vous pouvez préciser le nombre }
\CommentTok{\# d\textquotesingle{}échantillons bootstrap et donner une valeur de seed}
\NormalTok{interaction.gcomp }\OtherTok{\textless{}{-}} \FunctionTok{int.ltmleMSM}\NormalTok{(}\AttributeTok{data =}\NormalTok{ df,}
                                  ...
                                  \AttributeTok{Ynodes =} \FunctionTok{c}\NormalTok{(}\StringTok{"Y"}\NormalTok{),}
                                  \AttributeTok{SL.library =}\NormalTok{ SL.library,}
                                  \AttributeTok{gcomp =} \ConstantTok{TRUE}\NormalTok{, }\CommentTok{\# pour l\textquotesingle{}estimation par g{-}computation}
                                  \AttributeTok{iptw.only =} \ConstantTok{FALSE}\NormalTok{,}
                                  \AttributeTok{survivalOutcome =} \ConstantTok{FALSE}\NormalTok{,}
                                  \AttributeTok{variance.method =} \StringTok{"ic"}\NormalTok{,}
                                  \AttributeTok{B =} \DecValTok{1000}\NormalTok{, }\CommentTok{\# number of bootstrap samples}
                                  \AttributeTok{boot.seed =} \DecValTok{42}\NormalTok{) }\CommentTok{\# seed for bootstrap}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\# La fonction \textquotesingle{}estim.int.effects()\textquotesingle{} permet d\textquotesingle{}estimer les différentes quantités }
\DocumentationTok{\#\# d\textquotesingle{}intérêt pour l\textquotesingle{}analyse d\textquotesingle{}interaction à partir du MSM estimé précédemment.}
\CommentTok{\# Les IC95\% sont calculés par Delta méthode pour la TMLE et l\textquotesingle{}IPTW, et par }
\CommentTok{\# bootstrap pour la g{-}computation.}

\CommentTok{\# pour obtenir les estimations par TMLE : }
\NormalTok{est.tmle }\OtherTok{\textless{}{-}} \FunctionTok{estim.int.effects}\NormalTok{(interaction.ltmle, }\AttributeTok{estimator =} \StringTok{"tmle"}\NormalTok{)}

\CommentTok{\# pour obtenir les estimations par IPTW : }
\NormalTok{est.iptw }\OtherTok{\textless{}{-}} \FunctionTok{estim.int.effects}\NormalTok{(interaction.ltmle, }\AttributeTok{estimator =} \StringTok{"iptw"}\NormalTok{)}

\CommentTok{\# pour obtenir les estimations par g{-}computation : }
\NormalTok{est.gcomp }\OtherTok{\textless{}{-}} \FunctionTok{estim.int.effects}\NormalTok{(interaction.gcomp, }\AttributeTok{estimator =} \StringTok{"gcomp"}\NormalTok{)}

\DocumentationTok{\#\# Les résultats peuvent ensuite être présentés sous forme de table, en suivant }
\DocumentationTok{\#\# les recommandations de Knol et al. (2012) à l\textquotesingle{}aide de la fonction \textquotesingle{}out.int.table()\textquotesingle{}}

\CommentTok{\# par exemple pour les estimations par TMLE : }
\NormalTok{table\_inter }\OtherTok{\textless{}{-}} \FunctionTok{out.int.table}\NormalTok{(}\AttributeTok{int.r =}\NormalTok{ est.tmle)}
\NormalTok{table\_inter}\SpecialCharTok{$}\NormalTok{out.table}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# les mesures d\textquotesingle{}interaction sont disponible dans l\textquotesingle{}objet }
\NormalTok{table\_inter}\SpecialCharTok{$}\NormalTok{interaction.effects}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## [1] "additive Interaction = 0.394 [0.353;0.435]"
## [2] "RERI = 3.81 [3.38;4.3]"                    
## [3] "multiplicative Interaction = 1.15 [1;1.32]"
\end{verbatim}

On peut également se servir du package \texttt{KableExtra} pour mettre en forme les tables (selon les recommandations Knol et al. \citet{knol_recommendations_2012}) :
...
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(table\_inter}\SpecialCharTok{$}\NormalTok{out.table,) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{kable\_classic}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{add\_footnote}\NormalTok{(}\FunctionTok{c}\NormalTok{(table\_inter}\SpecialCharTok{$}\NormalTok{interaction.effects[}\DecValTok{1}\NormalTok{], }
\NormalTok{                 table\_inter}\SpecialCharTok{$}\NormalTok{interaction.effects[}\DecValTok{2}\NormalTok{], }
\NormalTok{                 table\_inter}\SpecialCharTok{$}\NormalTok{interaction.effects[}\DecValTok{3}\NormalTok{]),}
...
\hline
  & A2=0 & A2=1 & RD.A2|A1 & RR.A2|A1\\
\hline
A1=0 & \$p\_\{00\}\$=0.103 [0.095,0.112] & \$p\_\{01\}\$=0.196 [0.181,0.212] & 0.093 [0.075,0.11] & 1.9 [1.69,2.13]\\
\hline
A1=1 & \$p\_\{10\}\$=0.411 [0.38,0.442] & \$p\_\{11\}\$=0.898 [0.877,0.919] & 0.487 [0.449,0.524] & 2.18 [2.02,2.36]\\
\hline
RD.A1|A2 & 0.308 [0.276,0.34] & 0.702 [0.676,0.728] &  & \\
\hline
RR.A1|A2 & 3.98 [3.55,4.46] & 4.58 [4.22,4.97] &  & \\
\hline
\multicolumn{5}{l}{\textsuperscript{a} additive Interaction = 0.394 [0.353;0.435]}\\
\multicolumn{5}{l}{\textsuperscript{b} RERI = 3.81 [3.38;4.3]}\\
\multicolumn{5}{l}{\textsuperscript{c} multiplicative Interaction = 1.15 [1;1.32]}\\
\end{tabular}
\end{table}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{table\_inter }\OtherTok{\textless{}{-}}\NormalTok{ MargIntTmle}\SpecialCharTok{::}\FunctionTok{out.int.table}\NormalTok{(}\AttributeTok{int.r =}\NormalTok{ est.iptw)}
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(table\_inter}\SpecialCharTok{$}\NormalTok{out.table,) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{kable\_classic}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{add\_footnote}\NormalTok{(}\FunctionTok{c}\NormalTok{(table\_inter}\SpecialCharTok{$}\NormalTok{interaction.effects[}\DecValTok{1}\NormalTok{], }
\NormalTok{                 table\_inter}\SpecialCharTok{$}\NormalTok{interaction.effects[}\DecValTok{2}\NormalTok{], }
...
\hline
  & A2=0 & A2=1 & RD.A2|A1 & RR.A2|A1\\
\hline
A1=0 & \$p\_\{00\}\$=0.102 [0.093,0.111] & \$p\_\{01\}\$=0.199 [0.184,0.215] & 0.097 [0.079,0.115] & 1.95 [1.74,2.19]\\
\hline
A1=1 & \$p\_\{10\}\$=0.409 [0.378,0.44] & \$p\_\{11\}\$=0.9 [0.879,0.921] & 0.491 [0.454,0.529] & 2.2 [2.03,2.38]\\
\hline
RD.A1|A2 & 0.307 [0.274,0.339] & 0.701 [0.675,0.727] &  & \\
\hline
RR.A1|A2 & 4 [3.57,4.48] & 4.51 [4.16,4.89] &  & \\
\hline
\multicolumn{5}{l}{\textsuperscript{a} additive Interaction = 0.394 [0.353;0.436]}\\
\multicolumn{5}{l}{\textsuperscript{b} RERI = 3.86 [3.42;4.35]}\\
\multicolumn{5}{l}{\textsuperscript{c} multiplicative Interaction = 1.13 [0.98;1.3]}\\
\end{tabular}
\end{table}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{table\_inter }\OtherTok{\textless{}{-}}\NormalTok{ MargIntTmle}\SpecialCharTok{::}\FunctionTok{out.int.table}\NormalTok{(}\AttributeTok{int.r =}\NormalTok{ est.gcomp)}
\NormalTok{knitr}\SpecialCharTok{::}\FunctionTok{kable}\NormalTok{(table\_inter}\SpecialCharTok{$}\NormalTok{out.table,) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{kable\_classic}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{add\_footnote}\NormalTok{(}\FunctionTok{c}\NormalTok{(table\_inter}\SpecialCharTok{$}\NormalTok{interaction.effects[}\DecValTok{1}\NormalTok{], }
\NormalTok{                 table\_inter}\SpecialCharTok{$}\NormalTok{interaction.effects[}\DecValTok{2}\NormalTok{], }
...
\hline
  & A2=0 & A2=1 & RD.A2|A1 & RR.A2|A1\\
\hline
A1=0 & \$p\_\{00\}\$=0.104 [0.095,0.112] & \$p\_\{01\}\$=0.197 [0.183,0.211] & 0.093 [0.076,0.11] & 1.9 [1.7,2.13]\\
\hline
A1=1 & \$p\_\{10\}\$=0.4 [0.372,0.427] & \$p\_\{11\}\$=0.893 [0.872,0.915] & 0.494 [0.458,0.529] & 2.23 [2.08,2.4]\\
\hline
RD.A1|A2 & 0.296 [0.267,0.326] & 0.697 [0.67,0.723] &  & \\
\hline
RR.A1|A2 & 3.86 [3.45,4.32] & 4.54 [4.2,4.9] &  & \\
\hline
\multicolumn{5}{l}{\textsuperscript{a} additive Interaction = 0.4 [0.361;0.439]}\\
\multicolumn{5}{l}{\textsuperscript{b} RERI = 3.86 [3.46;4.32]}\\
\multicolumn{5}{l}{\textsuperscript{c} multiplicative Interaction = 1.18 [1.03;1.35]}\\
\end{tabular}
\end{table}
