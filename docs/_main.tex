...
\begin{quote}
Par exemple ici, \(\small V\) a un effet si \(\small X=0\) mais pas si \(\small X=1\) :
...
\begin{quote}
Par exemple ici, \(\small V\) a un effet positif si \(\small X=0\) mais négatif si \(\small X=1\) :
...
Si on donne 100 doses au groupe \(\small V = 0\), 30 personnes seront guéries grace au traitement (30 personnes de plus que l'évolution naturelle, \(\small X=0\)) contre 50 personnes si on les donne au groupe \(\small V = 1\).
Donc il est préférable d'allouer les doses au groupe \(\small V=1\).

Pourtant si on avait réfléchi à partir de l'échelle multiplicative, on aurait choisi le groupe \(\small V=0\) car l'effet du traitement est de RR=4 dans le groupe \(V = 0\) et RR=3,5 dans le groupe \(\small V = 1\)\ldots{}
\end{quote}
...
\hypertarget{pour-une-analyse-dune-modification-deffet-de-small-a1-sur-small-y-par-small-a2}{%
\subsection*{\texorpdfstring{Pour une analyse d'une modification d'effet de \(\small A1\) sur \(\small Y\) par \(\small A2\)}{Pour une analyse d'une modification d'effet de \textbackslash small A1 sur \textbackslash small Y par \textbackslash small A2}}\label{pour-une-analyse-dune-modification-deffet-de-small-a1-sur-small-y-par-small-a2}}
\addcontentsline{toc}{subsection}{Pour une analyse d'une modification d'effet de \(\small A1\) sur \(\small Y\) par \(\small A2\)}
...
\hypertarget{interaction-small-a1a2-sur-small-y}{%
\subsection*{\texorpdfstring{Interaction \(\small A1*A2\) sur \(\small Y\)}{Interaction \textbackslash small A1*A2 sur \textbackslash small Y}}\label{interaction-small-a1a2-sur-small-y}}
\addcontentsline{toc}{subsection}{Interaction \(\small A1*A2\) sur \(\small Y\)}
...
\item
  ...
  \item
    de l'effet de \(\small A1\) sur \(\small Y\) dans les strates de \(\small A2\)
  \item
    \textbf{et de \(\small A2\) sur \(\small Y\) dans les strates de \(\small A1\)}.
  \end{itemize}
\item
  ...
  \end{itemize}
\item
  Énumérez les facteurs de confusion pour lesquels la relation entre \(\small A1\) et \(\small Y\) \textbf{et la relation entre \(\small A2\) et \(\small Y\)} ont été ajustées.
\end{itemize}
