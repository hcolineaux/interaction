...
# Coefficients:
#                 Estimate Std. Error t value Pr(>|t|)    
# (Intercept)    1.3270453  0.0068655  193.29   <2e-16 ***
# sexeWomen     -0.3362871  0.0094614  -35.54   <2e-16 ***
# age           -0.0185252  0.0001335 -138.78   <2e-16 ***
# sexeWomen:age  0.0044007  0.0001853   23.75   <2e-16 ***# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
...
- L'effet du sexe (d'être femme plutot que homme) est : 
    + "A 0 ans" (coordonnée à l'origine) : $\small DR(S|A=0) = -33,63$%
    + A 40 ans (par exemple) : $\small DR(S|A=40) = -33,63 + 0,44 \times 40 =$ `r round(-33.63 + 0.44 * 40,1)`%
- L'effet de l'age est : 
    + Quand on est un homme : $\small DR(A|S=0) = -1,85$ % par année d'âge
    + Quand on est une femme : $\small DR(A|S=1) = -1,85 + 0.44 =$ `r round(-1.85 + 0.44,1)` % par année d'âge
- L'effet d'être une femme et d'avoir 1 an de plus 
    + plutot que homme et un certain âge
    + $\small DR(A,S) = -33,63 -1,85 + 0,44 =$ `r round(-33.7 -1.85 + 0.44,1)` %
- **L'effet d'interaction/modification d'effet** est :  $\small AI = +0.44$ %

On pourrait aussi déduire :

  - $\small Y_{S=0,A=30} = 132,7 - 1,85 \times 30 =$ `r round(132.7 - 1.85*30,1)`%
  - $\small Y_{S=0,A=50} = 132,7 - 1,85 \times 50 =$ `r round(132.7 - 1.85*50,1)`%
  - $\small Y_{S=1,A=30} = 132,7 -33,63 - 1,85 \times 30 + 0,44 \times 30 =$ `r round(132.7 - 33.63 - 1.85*30 + 0.44*30,1)`%
  - $\small Y_{S=1,A=50} = 132,7 -33,63 - 1,85 \times 50 + 0,44 \times 50 =$ `r round(132.7 - 33.63 - 1.85*50 +0.44*50,1)`%
...
# (Intercept)    3.9524082  0.0382785  103.25   <2e-16 ***
# sexeWomen     -1.1787828  0.0528351  -22.31   <2e-16 ***
# age           -0.0891173  0.0007808 -114.14   <2e-16 ***
# sexeWomen:age  0.0120384  0.0011038   10.91   <2e-16 ***
# ---
# Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
```
...
#                       OR      2.5 %     97.5 %
# (Intercept)   52.0605878 48.3044121 56.1245459
# sexeWomen      0.3076530  0.2773804  0.3412112
# age            0.9147383  0.9133364  0.9161362
# sexeWomen:age  1.0121112  1.0099239  1.0143033

```

- L'effet du sexe (d'être femme plutot que homme) est : 
    + "A 0 ans" (coordonnée à l'origine) : $\small OR(S|A=0) = \times 0.31$
    + "A 1 ans" : $\small OR(S|A=1) = \times 0.31 + 0,012 = \times$ `r round(exp(-1.178 + 0.012),1)`
    + A 40 ans (par exemple) : $\small OR(S|A=40) = exp(-1,179 + 0,012 \times 40) = \times$ `r round(exp(-1.178 + 0.012 * 40),1)`
- L'effet de l'age est : 
    + Quand on est un homme : $\small OR(A|S=0) = \times 0,91$ par année d'âge
    + Quand on est une femme : $\small OR(A|S=1) = exp(-0,089 + 0,012) = \times$ `r round(exp(-0.089 + 0.012),1)`  par année d'âge
- L'effet d'être une femme et d'avoir 1 an de plus 
    + plutot que homme "et 0 ans" 
    + $\small OR(A,S) = exp(-1,179 -0,089 + 0,012) = \times$ `r round(exp(-1.179 -0.089 + 0.012),1)`
- **L'effet d'interaction/modification d'effet** est :  $\small MI =\times 1,01$
- **Un effet d'interaction additif** $\small RERI_{OR} = OR_{11} - OR_{01} - OR_{10} + 1 =$ `r round(exp(-1.179 -0.089 + 0.012) - exp(-1.178 + 0.012) - exp(-0.089 + 0.012) + 1,3)`


On pourrait aussi déduire :

  - $\small Y_{S=0,A=30} = \frac{exp(3,952 - 0,089 \times 30)}{1+exp(3,952 - 0,089 \times 30)} =$ `r round((exp(3.952 - 0.089*30)/(1+exp(3.952 - 0.089*30)))*100,1)`%
  - $\small Y_{S=0,A=50} = \frac{exp(3,952 - 0,089 \times 50)}{1+exp(3,952 - 0,089 \times 50)} =$ `r round((exp(3.952 - 0.089*50)/(1+exp(3.952 - 0.089*50)))*100,1)`%
   - $\small Y_{S=1,A=30} = \frac{exp(3,952 - 1,179 - 0,089 \times 30 + 0,012 \times 30)}{1+exp(3,952 - 1,179 - 0,089 \times 30 + 0,012 \times 30)} =$ `r round((exp(3.952 - 1.179 - 0.089*30 +0.012*30)/(1+exp(3.952 - 1.179 - 0.089*30+0.012*30)))*100,1)`%
   - $\small Y_{S=1,A=50} = \frac{exp(3,952 - 1,179 - 0,089 \times 50 + 0,012 \times 50)}{1+exp(3,952 - 1,179 - 0,089 \times 50 + 0,012 \times 50)} =$ `r round((exp(3.952 - 1.179 - 0.089*50 +0.012*50)/(1+exp(3.952 - 1.179 - 0.089*50+0.012*50)))*100,1)`%
